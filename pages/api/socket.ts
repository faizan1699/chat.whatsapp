...
const SocketHandler = (req: NextApiRequest, res: NextApiResponse) => {
  if ((res.socket as SocketWithIO).server.io) {
    ...
    io.on('connection', (socket: SocketWithUsername) => {
      ...
      socket.on('create-conversation', async ({ participantIds, conversationName }) => {
        try {
            ...
            if (existingConvs?.length) {
              ...
              if (sharedConv) {
                const { data: conv } = await supabaseAdmin
                  ...
                  .single();
                if (conv) {
                  socket.emit('conversation-exists', conv);
                  return;
                }
              }
            }
          ...
          const conversationWithDetails = {
            ...newConv,
            participants: (usersData || []).map((u) => ({ user: u })),
            messages: []
          };

          participantIds.forEach((userId: string) => {
            const participantSocketId = Object.keys(allusers).find(key => allusers[key] === userId);
            if (participantSocketId) {
              io.to(participantSocketId).emit('new-conversation', conversationWithDetails);
            }
          });

          socket.emit('conversation-created', conversationWithDetails);
        } catch (error) {
